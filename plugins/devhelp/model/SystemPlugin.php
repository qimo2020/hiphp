...
class SystemPlugin extends Model
{
    ...
    public function design($data = [])
    {
        ...
        $data['status'] = 0;
        $data['icon'] = '/static/p_' . $data['name'] . '/images/' . $data['name'] . '.png';
        // 验证
        $valid = new \app\system\validate\SystemPlugin;
        ...
        mkdir('./static/plugin/' . $data['name'] . '/', 0777, true);
        // 生成插件信息
        ...
        copy('./static/mod/system/image/app.png', './static/plugins/' . $data['name'] . '/' . $data['name'] . '.png');
        // 生成说明文档
