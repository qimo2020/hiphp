...
class SystemLang extends Model
{
    ...
    public static function getDefaultLang($group){
        ...
        if(!$defaultLangs){
            ...
            if($defaultPack !== null && $res = self::where(['group'=>$group, 'pack'=>$defaultPack['id']])->select()->toArray()){
     ...
     * @author 祈陌 <3411869134@qq.com>
     */
    public static function langVals(string $group) : array {
        $res = self::getDefaultLang($group);
        $result = [];
        foreach ($res as $v){
            $result[$v['name']] = $v['langvar'];
        }
        return $result;
    }
