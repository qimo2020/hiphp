...
class SystemPlugin extends Model
{
    ...
    public static function getPlugins(){
        ...
        if(!$pluginInfos){
            if($res = self::select()->toArray()){
                cache('plugin_infos', json_encode($res));
                return $res;
            }
            return false;
        }
        return json_decode($pluginInfos, 1);
    }
    ...
    public function design($data = [])
    {
        ...
        $data['status'] = 0;
        $data['icon'] = '/static/plugin/' . $data['name'] . '/' . $data['name'] . '.png';
        // 验证
        ...
        copy('./static/m_system/images/app.png', './static/plugins/' . $data['name'] . '/' . $data['name'] . '.png');
        // 生成说明文档
