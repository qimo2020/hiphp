...
class Module extends Base
{
    ...
    protected function initialize()
    {
        ...
        $this->cloud = new Cloud(config('hi_cloud.identifier'), $this->tempPath);
    }
    ...
    public function index()
    {
        ...
        $map['system'] = 0;
        $modules = ModuleModel::where($map)->order('sort,id')->column('id,title,author,intro,icon,default,system,app_keys,identifier,name,version,status');
        if (0 == $status || 3 == $status) {
            ...
            if(3 == $status){
                if (!config('hi_cloud.identifier')) {
                    $tips = '你还没绑定应用中心,<a href="javascript:;" class="btn-bind-cloud"> <strong style="color:#428bca">立即绑定</strong> </a>';
                }else if(config('hi_cloud.expire') && config('hi_cloud.expire') < time()){
                    $tips = '绑定已过期,请<a href="javascript:;" class="btn-bind-cloud"> <strong style="color:#428bca">重新绑定</strong> </a>';
                }else{
                    $newModules = [];
                    foreach ($modules as $k=>$v){
                        $newModules[$k]['identifier'] = $v['identifier'];
                        $newModules[$k]['app_keys'] = $v['app_keys'];
                        $newModules[$k]['version'] = $v['version'];
                    }
                    $params = [''];
                    $cloudData = $this->cloud->data($params)->api('apps');
                    if(isset($cloudData['code']) && $cloudData['code'] == 0){
                        $tips = '未发现升级模块';
                    }
                }
                $tips = '<div class="hi-no-data-tips" style="padding: 50px 0;text-align: center">'.$tips.'</div>';
            }
        }
    ...
    public function install($id = 0)
    {
        if ($this->request->isPost()) {
            ...
            return $this->response(1,'模块已安装成功', url('index', ['status'=>2]));
        }
    ...
    public function import()
    {
        if ($this->request->isPost()) {
            ...
            Dir::delDir($decomPath);
            @unlink($file);
            return $this->response(1,'模块导入成功', url('', ['status'=>0]));
        }
        $tabData = $this->tabData;
        $tabData['current'] = url('');
        $this->assign('tabData', $tabData);
    ...
    public function uninstall()
    {
        ...
        if ($this->request->isPost()) {
            ...
            //更新缓存
            Cache::tag('menus')->clear();
            cache('module_infos', null);
            $this->response(1,'模块已卸载成功', url('index', ['status'=>0]));
        }
