        ...
        if ($status == 0) {
            ...
            foreach ($files as $k => $f) {
                ...
                if (file_exists($pluginsPath . $f . '/info.php')) {
                    ...
                    $sql['status'] = 0;
                    $sql['system'] = 0;
                    if(isset($info['app_keys']) && isset($info['app_id'])){
                        $sql['app_keys'] = $info['app_keys'];
                        $sql['app_id'] = $info['app_id'];
                    }
            ...
            if($pushs && $pushs[0]){
                foreach ($plugins as $k=>$v){
                    foreach ($pushs[0] as $kk=>$vv){
                        ...
                        if($status == 3){
                            if($v['name'] == $vv['app_name'] && $v['app_keys'] && $v['app_id'] && $v['app_keys'] == $vv['app_key'] && $v['app_id'] == $vv['app_id'] && version_compare($vv['version'], $v['version'],'>')){
                                $plugins[$k] = $v;
