...
class member extends Module
{
    ...
    public function installAfter()
    {
        ...
        $data = [
            ...
            ['title'=>'手机号', 'identifier'=>'phone', 'rule_hook'=>'verify_sms', 'rule_hook_tp'=>'verify_sms_tp|type:regist', 'rule'=>'require|regex:^1\d{10}','message'=>'请输入手机号|手机号格式不正确','check_after'=>1],
            ['title'=>'邮箱', 'identifier'=>'email', 'rule_hook'=>'', 'rule_hook_after'=>'email_regist_send', 'rule_hook_tp'=>'email_regist_tp', 'rule'=>'require|email','message'=>'请输入邮箱地址|邮箱格式不正确','check_after'=>1],
        ];
        $model->saveAll($data);
        return true;
    }
