layui.define(["element", "jquery"], function (exports) {
    ...
    layuihi = new function () {
        ...
        this.init = function (url) {
            ...
            $.getJSON(url, function (data, status) {
                if (data == null) {
                    layuihi.msg_error('暂无菜单信息');
                } else {
                    if(data.homeInfo){
                        layuihi.initHome(data.homeInfo);
                    }
                    if(data.logoInfo){
                        layuihi.initLogo(data.logoInfo);
                    }
                    if(data.clearInfo){
                        layuihi.initClear(data.clearInfo);
                    }
                    if(data.menuInfo){
                        layuihi.initMenu(data.menuInfo);
                    }
                    layuihi.initTab();
                }
            }).fail(function () {
                layuihi.msg_error('菜单接口有误');
            });
            layer.close(loading);
        };
        ...
        this.initHome = function (data) {
            sessionStorage.setItem('layuihiHomeHref', data.url);
            $('#layuihiHomeTabId').html('<i class="' + data.icon + '"></i> <span>' + data.title + '</span>');
            $('#layuihiHomeTabId').attr('lay-id', 0);
            $('#layuihiHomeTabIframe').html('<iframe width="100%" height="100%" frameborder="0" src="' + '/'+admin_path+'/'+data.url + '"></iframe>');
        };
        ...
        this.initClear = function (data) {
            $('.layuihi-clear').attr('data-href', data.clearUrl);
        };
        ...
        this.initBgColor = function () {
            var bgcolorId = sessionStorage.getItem('layuihiBgcolorId');
            if (bgcolorId == null || bgcolorId == undefined || bgcolorId == '') {
                bgcolorId = layuihi.config('BgColorDefault');
            }

            var bgcolorData = layuihi.bgColorConfig(bgcolorId);
            var styleHtml = '.layui-layout-admin .layui-header{background-color:' + bgcolorData.headerRight + '!important;}\n' +
                '.layui-header>ul>.layui-nav-item.layui-this,.layuihi-tool i:hover{background-color:' + bgcolorData.headerRightThis + '!important;}\n' +
                ...
                '}';
            $('#layuihi-bg-color').html(styleHtml);
        };
        ...
        this.initMenu = function (data) {
            ...
            $.each(data, function (key, val) {
                ...
                leftMenuHtml += '<ul class="layui-nav layui-nav-tree layui-hi-left-nav-tree ' + leftMenuCheckDefault + '" id="' + key + '">\n';
                var menuList = val.child;
                $.each(menuList, function (index, menu) {
                    leftMenuHtml += '<li class="layui-nav-item">\n';
                    if (menu.child != undefined && menu.child != [] && menu.url == '') {
                        ...
                        var buildChildHtml = function (html, child, menuParameId) {
                            ...
                            $.each(child, function (childIndex, childMenu) {
                                ...
                                if (childMenu.child != undefined && childMenu.child != []) {
                                    ...
                                    html += '<a href="javascript:;" class="layui-menu-tips" data-type="tabAdd" data-tab-id="'+menuParameId+'" data-tab-params="';
                                    if(childMenu.param != undefined && childMenu.param != ''){
                                        html += childMenu.param;
                                    }
                                    html += '" data-tab="' + childMenu.url + '" target="' + childMenu.target + '"><i class="' + childMenu.icon + '"></i><span class="layui-left-nav"> ' + childMenu.title + '</span></a>\n';
                                    menuParameId++;
                        ...
                        leftMenuHtml += '<a href="javascript:;" class="layui-menu-tips"  data-type="tabAdd" data-tab-id="'+menuParameId+'" data-tab-params="';
                        if(menu.param != undefined && menu.param != ''){
                            leftMenuHtml += menu.param;
                        }
                        leftMenuHtml += '" data-tab="' + menu.url + '" target="' + menu.target + '"><i class="' + menu.icon + '"></i><span class="layui-left-nav"> ' + menu.title + '</span></a>\n';
                        menuParameId++;
                    }
                    leftMenuHtml += '</li>\n';
                });
            ...
            $('.layui-header-hi-menu').html(headerMobileMenuHtml); //手机
        ...
        this.initTab = function () {
            ...
            if (urlArr.length >= 2) {
                ...
                // 判断链接是否有效
                var checkUrl = layuihi.checkUrl(href);
                if (checkUrl != true) {
                    return layuihi.msg_error(checkUrl);
                }

                // 判断tab是否存在
                var checkTab = layuihi.checkTab(href);
                if (!checkTab) {
                    var title = href,
                        tabId = href;
                    $("[data-tab]").each(function () {
                        ...
                        // 判断是否带参数了
                        if (layuihi.config('urlSuffixDefault')) {
                            var menuParames = $(this).attr('data-tab-params');
                            if(menuParames){
                                if (checkHref.indexOf("?") > -1) {
                                    checkHref = checkHref + '&' + menuParames;
                                } else {
                                    checkHref = checkHref + '?' + menuParames;
                                }
                            }
                            ...
                            var addMenuClass = function ($element, type) {
                                if (type == 1) {
                                    ...
                                    if ($element.attr('class') != 'layui-nav-item layui-this') {
                                        ...
                                        $(".layui-hi-left-nav-tree").attr('class', 'layui-nav layui-nav-tree layui-hide');
                    ...
                    var layuihiHomeTab = $('#layuihiHomeTab').attr('lay-id'),
                        layuihiHomeHref = sessionStorage.getItem('layuihiHomeHref');

                    // 非菜单打开的tab窗口
                    if (href == title) {
                        var layuihiTabInfo = JSON.parse(sessionStorage.getItem("layuihiTabInfo"));
                        if (layuihiTabInfo != null) {
                            var check = layuihiTabInfo[tabId];
                    ...
                    if (layuihiHomeTab != href && layuihiHomeHref != href) {
                        layuihi.addTab(tabId, href, title, true);
                        layuihi.changeTab(tabId);
                    }
                }
            }
            if (layuihi.config('urlHashLocation')) {
                layuihi.hashTab();
            }
        };
        ...
        this.buildBgColorHtml = function () {
            ...
            var bgColorConfig = layuihi.bgColorConfig();
        ...
        this.addTab = function (tabId, href, title, addSession) {
            if (addSession == undefined || addSession == true) {
                ...
                layuihiTabInfo[tabId] = {href: href, title: title}
                sessionStorage.setItem("layuihiTabInfo", JSON.stringify(layuihiTabInfo));
            }
            if(href != 0){
                element.tabAdd('layuihiTab', {
                    title: title + '<i data-tab-close="" class="layui-icon layui-unselect layui-tab-close">ဆ</i>'
                    , content: '<iframe width="100%" height="100%" allowfullscreen scrolling="auto" frameborder="0" src="' + '/'+admin_path+'/'+href + '"></iframe>'
                    , id: tabId
                });
            }
        ...
        this.delTab = function (tabId, isParent) {
            var layuihiTabInfo = JSON.parse(sessionStorage.getItem("layuihiTabInfo"));
            if (layuihiTabInfo != null) {
                delete layuihiTabInfo[tabId];
                sessionStorage.setItem("layuihiTabInfo", JSON.stringify(layuihiTabInfo))
            }
            if (isParent === true) {
                parent.layui.element.tabDelete('layuihiTab', tabId);
            } else {
                element.tabDelete('layuihiTab', tabId);
            }
        };
        ...
        this.changeTab = function (tabId) {
            element.tabChange('layuihiTab', tabId);
        };
        ...
        this.hashTab = function () {
            ...
            element.tabChange('layuihiTab', layId);
            element.on('tab(layuihiTab)', function (elem) {
        ...
        this.checkUrl = function (url) {
            if (!layuihi.config('checkUrlDefault')) {
                return true;
            }
            var msg = true;
            $.ajax({
    ...
    $('body').on('click', '[data-tab]', function () {
        ...
        // 判断该窗口是否已经打开过
        var checkTab = layuihi.checkTab(tabId);
        if (!checkTab) {
            layuihi.addTab(tabId, href, title, true);
        }
        element.tabChange('layuihiTab', tabId);
    ...
    $('body').on('click', '[data-iframe-tab]', function () {
        ...
        // 判断该窗口是否已经打开过
        var checkTab = layuihi.checkTab(tabId, true);
        if (!checkTab) {
            var layuihiTabInfo = JSON.parse(sessionStorage.getItem("layuihiTabInfo"));
            if (layuihiTabInfo == null) {
                layuihiTabInfo = {};
            ...
            layuihiTabInfo[tabId] = {href: href, title: title}
            sessionStorage.setItem("layuihiTabInfo", JSON.stringify(layuihiTabInfo));
            parent.layui.element.tabAdd('layuihiTab', {
        ...
        parent.layui.element.tabChange('layuihiTab', tabId);
        layuihi.tabRoll();
        parent.layer.close(loading);
    });
    ...
    $('body').on('click', '[data-clear]', function () {
        ...
        if (clearUrl != undefined && clearUrl != '' && clearUrl != null) {
            $.getJSON(clearUrl, function (data, status) {
                layer.close(loading);
                if (data.code != 1) {
                    return layuihi.msg_error(data.msg);
                } else {
                    return layuihi.msg_success(data.msg);
                }
            }).fail(function () {
                layer.close(loading);
                return layuihi.msg_error('清理缓存接口有误');
            });
        } else {
            layer.close(loading);
            return layuihi.msg_success('清除缓存成功');
        }
    });
    ...
    $('body').on('click', '[data-bgcolor]', function () {
        ...
        var bgColorHtml = layuihi.buildBgColorHtml();
        var html = '<div class="layuihi-color">\n' +
        ...
        layer.open({
            ...
            content: html,
                ...
                $('.layuihi-select-bgcolor').removeClass('layui-this');
            }
        });
        layer.close(loading);
    });
    ...
    $('body').on('click', '[data-select-bgcolor]', function () {
        ...
        $('.layuihi-color .color-content ul .layui-this').attr('class', '');
        $(this).attr('class', 'layui-this');
        sessionStorage.setItem('layuihiBgcolorId', bgcolorId);
        layuihi.initBgColor();
    });

    exports("layuihi", layuihi);
});
